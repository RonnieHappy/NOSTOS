...
The current manuscript is `docs/NOSTOS0_SOFTWARE_RESOURCE_ARTICLE.md`; its rendered Word
candidate is `docs/NOSTOS0_software_resource_submission_candidate.docx`. The terminal
claim, release and submission audit is `docs/NOSTOS0_FINAL_MAX_AUDIT.md`. The manuscript
is evidence-locked and production-QA complete, but it remains **not submission-ready**
while an archival DOI, unaided external execution, institutional determination and final
author declarations are outstanding. The flagship claim also remains blocked by failed
useful-coverage and matched-risk gates in the public bone stress program.
...
Run `nostos build-evidence-bundle --project-root . --output outputs/nostos0-evidence-bundle-v27` to regenerate the SHA-256 index of all 79 required evidence receipts. A complete index is an integrity result, not a Nature-readiness declaration. The claim-by-claim completion boundary is in `docs/NOSTOS0_FINAL_MAX_AUDIT.md`.
