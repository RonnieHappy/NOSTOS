...
Official comparators were executed in isolated, pinned environments. Kymatio 0.3.0 scattering reached balanced accuracy 0.875. PyRadiomics reached 1.00, equal to NOSTOS, and its installation reproduced all 14 evaluated first-order values from the IBSI digital phantom at the published three-significant-digit precision after the documented kurtosis convention was reconciled. Full texture-matrix IBSI conformance has not yet been established. The tie is important: the novelty claim rests on calibrated response geometry, perturbation accounting and abstention, not on defeating every feature library on a small synthetic classification task.
