...
**Audit state:** evidence locked, 28 August 2026  
...
| Structure tensor | Yes | Pass | Pristine PSHG local-orientation confirmation | scikit-image 0.25.2 cross-software pass | Externally and cross-software validated endpoint |
...
For a high-impact paper, this contract must produce a measurable advantage. The primary software-level hypothesis should be frozen as one of: fewer silently invalid outputs under acquisition shift, better calibrated selective risk, greater physical-scale consistency under resampling, or lower protocol divergence across domains. Classification accuracy is not the most natural endpoint and has already failed as the platform-wide story.
...
- a prospective demonstration that the measurement contract improves validity or reproducibility over focused upstream tools;
...
Accordingly, the present weakness is not a hidden pixel-level power defect. It is that the principal platform claim still lacks independently acquired biological confirmation and an unaided external-user result. Enlarging an already opened public archive would improve precision but would not close either independence gate.

## Final audit verdict

NOSTOS-0 is complete as an author-operated, public-data research tool for its bounded endpoints; it is not yet ready for a Nature-family submission. The public-data program now includes bulk and dense dynamic registration with calibrated uncertainty, externally confirmed network and spatial responses, BoneJ thickness concordance, cross-software tensor concordance, identity-disjoint focus confirmation and four end-to-end input contracts. The irreducible remaining gates are independent cartilage mask review if the OA application remains in the paper, an unaided external-user replication receipt, an archival release DOI and—if clinical or intraoperative language is desired—new prospective clinical evidence. The platform-level novelty claim must remain calibrated validity and abstention under a shared physical measurement contract, not universal classification.
