...
def measure_file(
    ...
    *,
    ...
    measurement_profile_path: Path | None = None,
) -> dict:
    ...
    measurement_profile = (
        None
        if measurement_profile_path is None
        else MeasurementProfile.from_path(measurement_profile_path)
    )
    ...
    payload["source"] = {
        ...
        "measurement_profile": (
            None if measurement_profile is None else measurement_profile.profile_id
        ),
    }
    ...
    evidence_counts = {
        status: sum(item["evidence_status"] == status for item in payload["responses"])
        for status in ("unvalidated", "developmental", "calibrated", "confirmed")
    }
    profile_provenance = payload["provenance"].get("measurement_profile")
    return {
        ...
        "measurement_profile": None if measurement_profile is None else measurement_profile.profile_id,
        "profile_compatibility": (
            None
            if profile_provenance is None
            else profile_provenance["machine_compatibility"]
        ),
    }
