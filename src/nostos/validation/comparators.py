...
def _ordered_responses(
    geometry, *, include_conditional_validated: bool = False
) -> list[tuple[str, np.ndarray]]:
    """Return a stable response list for legacy fixed-width benchmarks.

    Confirmed v2.6 endpoints are conditional by design: unsupported values are
    absent rather than imputed. They are therefore excluded from historical
    fixed-width representations unless a caller explicitly requests them and
    handles the support schema itself.
    """

    return sorted(
        (
            (
                f"{response.module}.{response.measurement}",
                np.asarray(response.values, dtype=float),
            )
            for response in geometry.responses
            if include_conditional_validated
            or not response.measurement.endswith("_v2_6")
        ),
