...
def canonical_response_blocks(geometry: ResponseGeometry, *, quotient_global_rotation: bool = True) -> dict[str, np.ndarray]:
    ...
    if spectral is not None:
        ...
        ordered_spectral = []
        if not quotient_global_rotation:
            ordered_spectral.append(("orientation", surfaces.get(("spectral", "orientation"))))
        ordered_spectral.extend(
            (
                ("anisotropy", surfaces.get(("spectral", "anisotropy"))),
                ("angular_entropy", surfaces.get(("spectral", "angular_entropy"))),
                ("spectral_slope", surfaces.get(("spectral", "spectral_slope"))),
                (
                    "characteristic_wavelength",
                    surfaces.get(("spectral", "characteristic_wavelength")),
                ),
            )
        )
        for _, values in ordered_spectral:
