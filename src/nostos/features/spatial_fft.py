...
class SpatialFFTFeatures:
    ...
    analysis_min_frequency_cycles_per_mm: float
    analysis_max_frequency_cycles_per_mm: float
...
def extract_spatial_fft(
    ...
    *,
    ...
    frequency_band_cycles_per_mm: tuple[float, float] | None = None,
) -> SpatialFFTFeatures:
    ...
    if frequency_band_cycles_per_mm is None:
        minimum_frequency = low_frequency_fraction * nyquist
        maximum_frequency = high_frequency_fraction * nyquist
    else:
        minimum_frequency, maximum_frequency = (float(value) for value in frequency_band_cycles_per_mm)
        if not 0 <= minimum_frequency < maximum_frequency:
            raise ValueError("The physical frequency band must satisfy 0 <= minimum < maximum.")
        tolerance = 32 * np.finfo(float).eps * max(1.0, nyquist)
        if maximum_frequency > nyquist + tolerance:
            raise ValueError(
                "The requested physical frequency band exceeds the image Nyquist frequency."
            )
    mask = (radius >= minimum_frequency) & (radius <= maximum_frequency)
    weights = power[mask]
    ...
    return SpatialFFTFeatures(
        ...
        analysis_min_frequency_cycles_per_mm=float(minimum_frequency),
        analysis_max_frequency_cycles_per_mm=float(maximum_frequency),
    )
