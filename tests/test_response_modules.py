...
def test_structure_tensor_recovers_programmed_orientation():
    ...
    assert min(response.orientation_resultant) > 0.8


def test_orientation_resultant_rejects_globally_incoherent_local_directions():
    y, x = np.mgrid[:256, :256]
    image = np.empty((256, 256), dtype=float)
    image[:128] = np.sin(2 * np.pi * x[:128] / 16.0)
    image[128:] = np.sin(2 * np.pi * y[128:] / 16.0)
    response = structure_tensor_response(image, spacing_um=(1, 1), scales_um=(1, 2, 4))
    assert min(response.coherency) > 0.75
    assert max(response.orientation_resultant) < 0.10
