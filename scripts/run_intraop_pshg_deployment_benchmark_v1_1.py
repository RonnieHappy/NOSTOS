...
from nostos.intraop.label_free_v1_1 import (
...
class _V11JsonAdapter:
    """Translate only the amended lock state for the immutable v1 evaluator."""

    @staticmethod
    def loads(value: str):
        payload = json.loads(value)
        if payload.get("schema_version") == "nostos-intraop-pshg-deployment-lock/1.1":
            if payload.get("status") != "locked_before_reference_error_evaluation":
                raise ValueError("The v1.1 lock is not frozen before reference-error evaluation.")
            payload = dict(payload)
            payload["amended_lock_status"] = payload["status"]
            # The v1 evaluator accepts one legacy state string. V1.1 preserves
            # the truthful state in the lock and translates it only in memory.
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/intraop_pshg_deployment_benchmark_v1_1.locked.json"),
    )
    parser.add_argument(
        "--lock",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("outputs/nostos0-intraop-pshg-deployment-v1_1"),
    )
