...
def build_lock(root: Path, dataset_root: Path, output: Path) -> dict:
    config_path = root / "configs/intraop_pshg_deployment_benchmark_v1_2.locked.json"
    config = json.loads(config_path.read_text(encoding="utf-8"))
    prior_lock_path = root / config["lineage"]["v1_1_lock_path"]
    if (
        prior_lock_path.stat().st_size != int(config["lineage"]["v1_1_lock_bytes"])
        or _sha256(prior_lock_path) != config["lineage"]["v1_1_lock_sha256"]
    ):
        raise ValueError("The v1.1 lock does not match the amendment lineage.")
    failure_path = root / config["lineage"]["v1_1_failure_path"]
    if (
        failure_path.stat().st_size != int(config["lineage"]["v1_1_failure_bytes"])
        or _sha256(failure_path) != config["lineage"]["v1_1_failure_sha256"]
    ):
        raise ValueError("The v1.1 failure receipt does not match the amendment lineage.")
    prior_lock = json.loads(prior_lock_path.read_text(encoding="utf-8"))
    if list(prior_lock["selection"]["selected_rois"]) != list(config["selection"]["selected_rois"]):
        raise ValueError("The v1.2 amendment changed the locked ROI selection.")
    manifest_path = dataset_root / config["dataset"]["manifest_relative_path"]
    manifest_sha256 = _sha256(manifest_path)
    if manifest_sha256 != prior_lock["dataset_manifest"]["sha256"]:
        raise ValueError("The public download manifest changed after the v1.1 lock.")
    locked_files = []
    ...
    payload = {
        "schema_version": "nostos-intraop-pshg-deployment-lock/1.2",
        "status": "locked_before_reference_error_evaluation",
        "protocol_version": config["protocol_version"],
        "selection": config["selection"],
        "amendment": config["amendment"],
        "lineage": config["lineage"],
        "dataset_manifest": {**prior_lock["dataset_manifest"], "sha256": manifest_sha256},
        "locked_files": locked_files,
        "selected_source_files": prior_lock["selected_source_files"],
        "access_state": "The v1 and v1.1 runs stopped before reference-error computation. V1.2 is frozen before any reference error or success gate is computed.",
        ...
        json.dumps(
            {
                ...
                "files": len(result["locked_files"]),
                "source_files": len(result["selected_source_files"]),
            },
            indent=2,
        )
    )
