"""Execute and repeat the frozen NOSTOS selective-risk baseline audit."""
...
from nostos.validation.selective_risk_baseline import (
    build_selective_risk_baseline_audit,
)


ROOT = Path(__file__).resolve().parents[1]
PRIMARY = ROOT / "outputs/nostos0-selective-risk-baseline-v1/selective_risk_baseline.json"
...
def _finalize(payload: dict, repeat_identical: bool) -> dict:
    ...
    before = dict(result.pop("success_gates_before_repeat"))
    gates = {**before, "byte_identical_independent_repeat": repeat_identical}
...
def main() -> None:
    first = build_selective_risk_baseline_audit(ROOT, draws=5000)
    second = build_selective_risk_baseline_audit(ROOT, draws=5000)
