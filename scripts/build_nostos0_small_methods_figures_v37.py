"""Build the journal-density NOSTOS Small Methods v37 figures.

The figure geometry is derived from a complete visual audit of three supplied
2026 Small Methods research articles and the current Wiley graphics guidance.
Authentic public microscopy remains visually dominant.  All maps and numerical
summaries are deterministic; no generated microscopy or result map is used.
"""
...
import build_nostos0_small_methods_figures_v36 as old


ROOT = old.ROOT
OUT = ROOT / "figures" / "nostos0_small_methods_v37"
old.OUT = OUT
old.sm.OUT = OUT
old.v35.OUT = OUT
...
PALE = old.PALE
TEAL = old.TEAL
TEAL_LIGHT = old.TEAL_LIGHT
BLUE = old.BLUE
RED = old.RED
RED_DARK = old.RED_DARK
AMBER = old.AMBER
WHITE = old.WHITE
...
mpl.rcParams.update(
    {
        ...
        "svg.fonttype": "none",
        "svg.hashsalt": "nostos-small-methods-v37",
        "pdf.fonttype": 42,
...
def panel(ax: mpl.axes.Axes, letter: str, *, inside: bool = False) -> None:
    ...
    writer(
        0.018 if inside else -0.055,
        0.982 if inside else 1.025,
        letter,
...
def image_panel(
    ...
    *,
    ...
    out = ax.imshow(image, cmap=cmap, vmin=vmin, vmax=vmax, interpolation="nearest")
    ax.set_xticks([])
    ax.set_yticks([])
...
def clean_axes(ax: mpl.axes.Axes, *, grid: bool = True) -> None:
    ...
    ax.tick_params(length=2.2, width=0.6, color=LIGHT, pad=1.7)
    if grid:
        ax.grid(axis="y", color=PALE, linewidth=0.55, zorder=0)
    ax.margins(x=0.02)


def support_matrix(ax: mpl.axes.Axes, supported: set[tuple[str, float]], *, compact: bool = False) -> None:
    ...
    colors[matrix == 0] = mpl.colors.to_rgba("#E9EDF0")
    ...
    for row in range(5):
        for column in range(3):
            ax.text(
                ...
                fontsize=7.0 if compact else 7.7,
                fontweight="bold",
            )
...
def measurement_rail(
    ...
    """Data-bearing rail using the restrained geometry explored in BioRender."""
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.axis("off")
    objects = [(0.01, 0.24, 0.20, 0.52), (0.28, 0.24, 0.20, 0.52)]
    for (x, y, width, height), data, cmap in (
        (objects[0], vf.robust_unit(image), "gray"),
        (objects[1], orientation, None),
    ):
        inset = ax.inset_axes([x, y, width, height])
        inset.imshow(data, cmap=cmap, interpolation="nearest")
        inset.set_axis_off()
    lattice = ax.inset_axes([0.57, 0.24, 0.14, 0.52])
    support_matrix(lattice, supported, compact=True)
    for start, end in ((0.21, 0.275), (0.48, 0.565)):
        ax.add_patch(
            FancyArrowPatch(
                (start, 0.50),
                (end, 0.50),
    ...
    ax.plot([0.71, 0.78], [0.50, 0.50], color=MID, linewidth=0.75)
    ax.plot([0.78, 0.78], [0.34, 0.66], color=MID, linewidth=0.75)
    for y in (0.34, 0.66):
        ax.add_patch(
            FancyArrowPatch((0.78, y), (0.82, y), arrowstyle="-|>", mutation_scale=7, linewidth=0.75, color=MID)
        )
    ax.add_patch(Rectangle((0.83, 0.56), 0.14, 0.20, facecolor=WHITE, edgecolor=TEAL, linewidth=1.0))
    ax.add_patch(Rectangle((0.83, 0.24), 0.14, 0.20, facecolor=WHITE, edgecolor=LIGHT, linewidth=1.0))
    ax.text(0.90, 0.66, "●", ha="center", va="center", color=TEAL, fontsize=9.0)
    ax.text(0.90, 0.34, "×", ha="center", va="center", color=MID, fontsize=8.5, fontweight="bold")
...
def figure1(
    ...
    fig = plt.figure(figsize=(7.01, 3.55))
    fig.subplots_adjust(left=0.060, right=0.995, top=0.968, bottom=0.105, hspace=0.24, wspace=0.44)
    gs = fig.add_gridspec(2, 14, height_ratios=[1.02, 0.98])
    top = gs[0, :].subgridspec(1, 7, wspace=0.035)
    displays = [
        (vf.robust_unit(bio_input), "input", "gray"),
        (vf.robust_unit(bio_reference), "paired reference", "gray"),
        (bio_orientation, "orientation", None),
        (bio_coherence, "coherence", "viridis"),
        (vf.robust_unit(fmd_input), "input", "gray"),
    ...
    for index, (data, label, cmap) in enumerate(displays):
        ...
        image = image_panel(ax, data, label, cmap=cmap, vmin=0.0 if coherence else None, vmax=1.0 if coherence else None)
        if index == 0:
            panel(ax, "a", inside=True)
            ax.text(0, 1.10, "BioSR", transform=ax.transAxes, ha="left", va="bottom", fontsize=6.8, fontweight="bold")
        if index == 4:
            panel(ax, "b", inside=True)
            ax.text(0, 1.10, "FMD", transform=ax.transAxes, ha="left", va="bottom", fontsize=6.8, fontweight="bold")
        if coherence:
            mappable = image
    color_axis = fig.add_axes([0.897, 0.525, 0.082, 0.008])
    colorbar = fig.colorbar(mappable, cax=color_axis, orientation="horizontal")
    colorbar.set_ticks([0, 1])
    colorbar.ax.tick_params(labelsize=5.0, length=1.0, pad=0.5)
    ...
    ax.plot(scales, reference, "o-", color=INK, linewidth=1.15, markersize=2.9, label="reference")
    ax.plot(scales, estimate, "o-", color=TEAL, linewidth=1.45, markersize=2.9, label="NOSTOS")
    ax.fill_between(scales, reference, estimate, color=TEAL_LIGHT, alpha=0.30, linewidth=0)
...
def figure2(
    ...
    fig = plt.figure(figsize=(7.01, 3.85))
    fig.subplots_adjust(left=0.073, right=0.995, top=0.970, bottom=0.105, hspace=0.23, wspace=0.45)
    gs = fig.add_gridspec(2, 12, height_ratios=[1.10, 1.08])
    images = gs[0, :].subgridspec(1, 5, wspace=0.035)
    for index, (data, label, cmap) in enumerate(
        [
        ...
        image_panel(ax, data, label, cmap=cmap)
        if index == 0:
            panel(ax, "a", inside=True)
    ...
    ax.barh(y, valid_fraction, height=0.64, color=TEAL, label="valid")
    ax.barh(y, invalid_fraction, left=valid_fraction, height=0.64, color=RED, label="invalid")
    ax.barh(y, abstain_fraction, left=np.array(valid_fraction) + np.array(invalid_fraction), height=0.64, color=LIGHT, label="withheld")
    ax.set_yticks(y, sm.CONDITION_LABELS)
    ax.invert_yaxis()
    ax.set_xlim(0, 1)
    ax.set_xlabel("fraction")
    ax.legend(ncol=3, loc="lower center", bbox_to_anchor=(0.5, 1.01), columnspacing=0.8, handlelength=1.2)
    ...
    for group in groups:
        ...
        ax.plot([0, 1], values, color=LIGHT, linewidth=0.85, zorder=1)
        ax.scatter([0, 1], values, color=[BLUE, TEAL], s=17, edgecolor=WHITE, linewidth=0.35, zorder=3)
    ...
    ax.plot(qx, qy, color=BLUE, linewidth=1.25, label="QC")
    ax.plot(nx, ny, color=TEAL, linewidth=1.55, label="NOSTOS")
    ax.scatter([qc["coverage"], full["coverage"]], [qc["risk"], full["risk"]], color=[BLUE, TEAL], s=24, edgecolor=WHITE, linewidth=0.4, zorder=4)
...
def support_repair(ax: mpl.axes.Axes) -> None:
    labels = ["16·4", "16·8", "16·16", "8·16"]
    before = np.array([[1, 1, 1, -1]])
    after = np.array([[1, 1, 1, 0]])
    array = np.vstack([before, after])
    palette = {1: TEAL, 0: LIGHT, -1: RED}
    rgba = np.empty((*array.shape, 4), dtype=float)
    for value, color in palette.items():
        rgba[array == value] = mpl.colors.to_rgba(color)
...
def figure3(
    ...
    fig = plt.figure(figsize=(7.01, 3.55))
    fig.subplots_adjust(left=0.060, right=0.995, top=0.970, bottom=0.115, hspace=0.24, wspace=0.48)
    gs = fig.add_gridspec(2, 14, height_ratios=[1.03, 0.97])
    ...
    for index, level in enumerate(("raw", "avg2", "avg4", "avg8", "avg16", "avg50")):
        ...
        image_panel(ax, vf.robust_unit(fmd["ladder"][level], 1, 99.5), level.replace("avg", "×"), cmap="gray")
        if index == 0:
            panel(ax, "a", inside=True)
    ...
    for index, field in enumerate(fields):
        ...
        for repeat_index, row in enumerate(subset):
            ...
            ax.scatter(index + jitter, float(row["error"]), s=18, color=RED if bool(row["invalid"]) else TEAL, edgecolor=WHITE, linewidth=0.35, zorder=3)
    ...
    ax.errorbar(rate, 1, xerr=[[rate - low], [high - rate]], fmt="o", color=RED, ecolor=INK, elinewidth=0.9, capsize=3.0, markersize=4.2)
    ax.errorbar(0.0, 0, xerr=[[0.0], [upper]], fmt="o", color=TEAL, ecolor=INK, elinewidth=0.9, capsize=3.0, markersize=4.2)
    ax.set_yticks([0, 1], ["v1.6\n0/19", "v1.5\n2/7"])
...
def figure4(
    ...
    fig = plt.figure(figsize=(7.01, 3.55))
    fig.subplots_adjust(left=0.062, right=0.995, top=0.970, bottom=0.115, hspace=0.25, wspace=0.46)
    gs = fig.add_gridspec(2, 15, height_ratios=[1.20, 0.80])
    ...
    for group, (source, label) in enumerate(sources):
        ...
        image_panel(ax, vf.robust_unit(source, 1, 99.5), label, cmap="gray")
        if group == 0:
            panel(ax, "a", inside=True)
        ax = fig.add_subplot(top[0, group * 2 + 1])
        mappable = image_panel(ax, coherence, "coherence", cmap="viridis", vmin=0, vmax=1)
    color_axis = fig.add_axes([0.898, 0.526, 0.082, 0.008])
    colorbar = fig.colorbar(mappable, cax=color_axis, orientation="horizontal")
    colorbar.set_ticks([0, 1])
    colorbar.ax.tick_params(labelsize=5.0, length=1.0, pad=0.5)
    colorbar.outline.set_linewidth(0.3)
    ...
    for source in source_order:
        for field in summaries[source]["fields"]:
            ...
            if accepted_count == 0:
                ax.scatter(x, 0, marker="x", color=MID, s=20, linewidth=0.9)
            else:
                risk = invalid_count / accepted_count
                ax.scatter(x, risk, color=RED if risk else TEAL, s=23, edgecolor=WHITE, linewidth=0.35)
            ticks.append(x)
            labels.append(field["reference_group_id"].split("fov")[-1])
            x += 1
        x += 1
    ax.axhline(0.15, color=RED_DARK, linewidth=0.75, linestyle=(0, (3, 2)))
    ax.axvline(7, color=LIGHT, linewidth=0.7)
    ...
    ax.bar([0, 1], [before["accepted"], after["accepted"]], color=[AMBER, LIGHT], width=0.54)
    ax.bar([0, 1], [before["invalid"], after["invalid"]], color=[RED, RED], width=0.54)
    ...
    ax.text(1, 3, "0", ha="center", color=MID, fontweight="bold")
    clean_axes(ax)
    ...
    for index, (label, in_scope, emit) in enumerate(rows):
        ...
        for x_value, state in ((0.55, in_scope), (1.55, emit)):
            ax.scatter(x_value, y, s=43, color=TEAL if state else LIGHT, edgecolor=WHITE, linewidth=0.35)
            ax.text(x_value, y, "●" if state else "×", ha="center", va="center", color=WHITE if state else MID, fontweight="bold")
...
def toc_figure(fmd: Mapping[str, Any], strict_profile: Mapping[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    fig = plt.figure(figsize=(4.3307, 0.7874))
    ax = fig.add_axes([0, 0, 1, 1])
    measurement_rail(ax, image, orientation, supported)
    ...
    for suffix, kwargs in (
        ...
        outputs[suffix] = {"path": path.relative_to(ROOT).as_posix(), "bytes": path.stat().st_size, "sha256": sha256(path)}
    plt.close(fig)
    return outputs
...
def build_manifest(outputs: Mapping[str, Any], sources: Sequence[Path], archives: Sequence[Path]) -> Path:
    payload = {
        "schema_version": "nostos-small-methods-figures/1.4",
        "status": "complete",
        "generated_by": Path(__file__).relative_to(ROOT).as_posix(),
        "generated_by_sha256": sha256(Path(__file__)),
        "declaration": "Every microscopy pixel originates in a cited public BioSR or FMD archive. All maps and summaries are deterministic. BioRender supplied only a data-free composition study; no generated microscopy, anatomy, mask, heatmap or numerical result appears.",
    ...
    OUT.mkdir(parents=True, exist_ok=True)
    path = OUT / "small_methods_figures_v37.manifest.json"
...
def main() -> None:
    ...
    confocal_image, _ = old.v35.load_external_pair(confocal_archive, external_rows, source_key="Confocal_BPAE_R", field=7)
    widefield_g_image, _ = old.v35.load_external_pair(widefield_g_archive, external_rows, source_key="WideField_BPAE_G", field=1)
    outputs = {
