...
LOCKED_PATHS = (
    ...
    "configs/intraop_pshg_deployment_benchmark_v1_4.locked.json",
    "configs/intraop_pshg_deployment_v1_3_audit.locked.json",
    ...
    "manifests/intraop_pshg_deployment_v1_3_lock.json",
    ...
    "outputs/nostos0-intraop-pshg-deployment-v1_3/deployment_benchmark.json",
    "outputs/nostos0-intraop-pshg-deployment-v1_3-final-audit/final_audit.json",
    ...
    "src/nostos/intraop/label_free_v1_4.py",
    ...
    "scripts/run_intraop_pshg_deployment_benchmark_v1_4.py",
    "scripts/audit_intraop_pshg_deployment_v1_3.py",
    ...
    "scripts/build_intraop_pshg_deployment_v1_4_lock.py",
    ...
    "tests/test_intraop_label_free_v1_4.py",
...
def _verify_lineage(root: Path, config: dict) -> dict:
    observed = {}
    for stem in ("v1_3_lock", "v1_3_result", "v1_3_audit_config", "v1_3_audit_script", "v1_3_audit_result"):
        path = root / config["lineage"][f"{stem}_path"]
        size = int(config["lineage"][f"{stem}_bytes"])
        digest = config["lineage"][f"{stem}_sha256"]
        if path.stat().st_size != size or _sha256(path) != digest:
            raise ValueError(f"The {stem} artifact does not match v1.4 lineage.")
        observed[stem] = {"path": path.relative_to(root).as_posix(), "bytes": size, "sha256": digest}
    audit = json.loads((root / config["lineage"]["v1_3_audit_result_path"]).read_text(encoding="utf-8"))
    if audit["status"] != "audit_fail" or audit["checks"].get("artifact_registry_complete") is not False:
        raise ValueError("V1.4 requires the exact v1.3 artifact-registry audit failure.")
    return observed


def _selection(config: dict, dataset_root: Path, prior_rois: set[str]) -> list[str]:
    salt = config["selection"]["salt"]
    candidates = [path.name for path in dataset_root.glob("breast_*") if path.is_dir() and path.name not in prior_rois]
    ...
    if selected != list(config["selection"]["selected_rois"]) or digests != list(config["selection"]["selection_sha256"]):
        raise ValueError("The v1.4 field selection does not reproduce from the locked rule.")
    return selected
...
def _source_files(dataset_root: Path, selected: list[str]) -> list[dict]:
    ...
    for roi in selected:
        ...
        for path in expected:
            records.append({"relative_path": path.relative_to(dataset_root).as_posix(), "bytes": path.stat().st_size, "sha256": _sha256(path)})
    return records


def build_lock(root: Path, dataset_root: Path, output: Path) -> dict:
    config_path = root / "configs/intraop_pshg_deployment_benchmark_v1_4.locked.json"
    config = json.loads(config_path.read_text(encoding="utf-8"))
    lineage = _verify_lineage(root, config)
    v12 = json.loads((root / "manifests/intraop_pshg_deployment_v1_2_lock.json").read_text(encoding="utf-8"))
    v13 = json.loads((root / config["lineage"]["v1_3_lock_path"]).read_text(encoding="utf-8"))
    selected = _selection(config, dataset_root, set(v12["selection"]["selected_rois"]) | set(v13["selection"]["selected_rois"]))
    manifest_path = dataset_root / config["dataset"]["manifest_relative_path"]
    if _sha256(manifest_path) != v13["dataset_manifest"]["sha256"]:
        raise ValueError("The public download manifest changed after the v1.3 lock.")
    locked_files = []
    ...
    payload = {
        "schema_version": "nostos-intraop-pshg-deployment-lock/1.4",
        "status": "locked_before_v1_4_pixel_decode",
        ...
        "lineage": lineage,
        "dataset_manifest": {**v13["dataset_manifest"], "sha256": _sha256(manifest_path)},
        "locked_files": locked_files,
        "selected_source_files": _source_files(dataset_root, selected),
        "access_state": "V1.4 code, gates, selection and source hashes were frozen before selected v1.4 pixel arrays or FI reference values were decoded by the deployment benchmark.",
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("manifests/intraop_pshg_deployment_v1_4_lock.json"))
